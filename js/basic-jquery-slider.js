 ...
 */ (function ($) {
    $.fn.bjqs = function (options) {
        ...
        if (settings.hoverPause && settings.automatic) {
            ...
            $container.hover(function () {
                if (!paused) {
                    clearInterval(bjqsInterval);
                    paused = true;
					// Maybe add on-hover effect here?
                }
            }, function () {
                if (paused) {
                    bjqsInterval = setInterval(function () {
        ...
        if (settings.animation === 'slide' && slideCount > 1) {
            ...
            $second_last = slides.eq(slideCount - 2);

            $first.clone().addClass('clone').removeClass('slide').appendTo($slider);
			$second.clone().addClass('clone').removeClass('slide').appendTo($slider);
            $last.clone().addClass('clone').removeClass('slide').prependTo($slider);
            $second_last.clone().addClass('clone').removeClass('slide').prependTo($slider);
        ...
        slides.eq(current+1).addClass('current-slide');
        $slider.show();
        ...
        var bjqsGo = function (direction, position) {
                ...
                if (!animating) {
                    ...
                    if (settings.animation === 'fade') {
						...
						slides.eq(slidePosition).removeClass('current-slide');

						if (next === slideCount - 1) {
							slides.eq(1).addClass('current-slide');
						} else if (next === 0) {
							slides.eq(slideCount - 2).addClass('current-slide');
						} else {
							slides.eq(next).addClass('current-slide');
						}
